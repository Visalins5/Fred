...
def load_data():
    ...
    try:
        conn = psycopg2.connect(
            ...
            password=os.environ.get("DB_PASSWORD", "postgrespv"),
...
def create_slicing_charts(df):
    ...
    # Filter presidents who served during the selected interval
    selected_presidents = [
        f"{p['name']} ({p['start']}-{p['end']})"
        for p in presidents
        if not (p['end'] < year_range[0] or p['start'] > year_range[1])
    ]
    presidents_text = "\n".join(selected_presidents) if selected_presidents else "No presidents found for this interval."
    st.text_area(
        "U.S. Presidents during selected interval:",
        presidents_text,
        height=100,
        key=f"presidents_{year_range[0]}_{year_range[1]}"
    )
...
def create_dicing_charts(df):
    ...
    key_events = {
        2010: {
            'Q1': "Slow recovery from the Great Recession; unemployment remains high.",
            'Q2': "Private sector job growth resumes; stimulus spending continues.",
            'Q3': "Manufacturing rebounds; housing market stabilizes.",
            'Q4': "Holiday hiring boosts employment; GDP growth positive."
        },
        2020: {
            'Q1': "COVID-19 pandemic begins; initial job losses in March.",
            'Q2': "Massive layoffs due to lockdowns; unemployment spikes.",
            'Q3': "Partial reopening; job recovery starts.",
            'Q4': "Vaccine news; holiday season sees mixed job trends."
        },
        2021: {
            'Q1': "Vaccine rollout accelerates; job growth resumes.",
            'Q2': "Reopening of many sectors; strong job gains.",
            'Q3': "Delta variant slows recovery; labor shortages.",
            'Q4': "Omicron concerns; continued job growth."
        },
        2022: {
            'Q1': "Strong labor market; wage growth.",
            'Q2': "Inflation concerns; Fed rate hikes begin.",
            'Q3': "Labor force participation rises.",
            'Q4': "Holiday hiring; tech layoffs start."
        },
        2023: {
            'Q1': "Tech layoffs continue; service sector strong.",
            'Q2': "Job growth moderates; inflation eases.",
            'Q3': "Labor market stabilizes.",
            'Q4': "Year ends with low unemployment."
        },
        2024: {
            'Q1': "Steady job growth; AI sector expands.",
            'Q2': "Manufacturing investments increase.",
            'Q3': "Consumer spending supports jobs.",
            'Q4': "Election year hiring trends."
        },
        2025: {
            'Q1': "Post-election policy changes impact jobs.",
            'Q2': "Green energy hiring accelerates.",
            'Q3': "Healthcare sector leads growth.",
            'Q4': "Year closes with stable employment."
        }
        # Add more years/quarters as needed
    }

    # Collect events for the selected year range and quarter
    events_text = ""
    for year in range(year_range[0], year_range[1] + 1):
        if year in key_events and quarter in key_events[year]:
            events_text += f"<b>{year} {quarter}:</b> {key_events[year][quarter]}<br>"

    if not events_text:
        events_text = "<b>No key events available for the selected interval and quarter.</b>"

    st.markdown(
        f"""
        <div style="background-color:#d4f7d4; padding:18px; border-radius:10px; margin-top:16px;">
...
def create_roll_up_charts(df):
    ...
    # Roll-up 3: Total Jobs Created by Decade (1940–2020)
    st.subheader("Total Jobs Created by Decade (1940–2020)")

    # Ensure 'year' column exists
    df['year'] = df['date'].dt.year

    # Create a 'decade' column (e.g., 1940s, 1950s, ...)
    df['decade'] = (df['year'] // 10) * 10

    # Filter for decades from 1940 to 2020
    df_decades = df[(df['decade'] >= 1940) & (df['decade'] <= 2020)].copy()

    # Group by decade and sum total jobs created
    decade_jobs = df_decades.groupby('decade')['total_nonfarm'].sum().reset_index()

    # Convert decade to string for better labeling (e.g., "1940s")
    decade_jobs['Decade'] = decade_jobs['decade'].astype(str) + "s"

    # Create a color sequence for each bar
    color_sequence = px.colors.qualitative.Plotly
    # If there are more decades than colors, repeat the color list
    colors = color_sequence * ((len(decade_jobs) // len(color_sequence)) + 1)

    fig_decade_sum = px.bar(
        decade_jobs,
        x='Decade',
        y='total_nonfarm',
        color='Decade',
        color_discrete_sequence=colors,
        title="Total Jobs Created by Decade (1940–2020)",
        labels={'total_nonfarm': 'Total Jobs Created (in thousands)', 'Decade': 'Decade'}
    )
    st.plotly_chart(fig_decade_sum, use_container_width=True)
